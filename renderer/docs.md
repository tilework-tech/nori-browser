...
- Frontend UI for the Nori Browser Electron app: a terminal sidebar on the left, a draggable divider, and a browser toolbar on the right
- `renderer.js` is the source; it gets bundled by esbuild into `bundle.js` which is loaded by `index.html`
- All communication with the main process goes through `window.api` (exposed via `preload.js` in `@/`)
