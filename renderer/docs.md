...
- Frontend UI for the Nori Browser Electron app: a terminal sidebar on the left, a draggable divider, a browser toolbar with URL bar, a Chrome-like tab bar, and several overlay UI elements (find bar, download shelf, status bar, zoom indicator) on the right
...
- Sends user actions back to main process: terminal keystrokes (`sendTerminalInput`), URL bar navigation (`navigate`), toolbar buttons (`goBack`, `goForward`, `reload`), sidebar width changes (`resizeSidebar`), sidebar toggle (`toggleSidebar`), tab operations (`createTab`, `closeTab`, `switchTab`, `reorderTab`, `reopenClosedTab`, `duplicateTab`, `closeOtherTabs`, `closeTabsToRight`, `showTabContextMenu`, `pinTab`, `unpinTab`), find-in-page commands (`findInPage`, `findNext`, `findClose`), and download actions (`downloadCancel`, `downloadOpen`, `downloadShow`)
