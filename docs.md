...
- **Tab system**: Each tab is a `WebContentsView` with its own `webContents` and process, managed via module-level `tabs` array, `activeTabId`, and `nextTabId` counter. Each tab object has `{ id, view, title, url, pinned }` where `pinned` defaults to `false`. `createTab()` creates a view, adds it to `mainWindow.contentView`, wires navigation/title events, and switches to it. `createTab()` accepts an optional `insertIndex` parameter for position-controlled insertion (used by reopen and duplicate). Tab switching uses `view.setVisible(true/false)` rather than add/remove to avoid flicker. Closing the last tab closes the window
