...
- **IPC channels** in `preload.js` expose a `window.api` surface organized into groups: terminal I/O, navigation, layout, state sync, tab management, find-in-page (`findInPage`, `findNext`, `findClose`, `onFindResults`, `onShowFindBar`), zoom (`onZoomChanged`), downloads (`onDownloadProgress`, `onDownloadDone`, `downloadCancel`, `downloadOpen`, `downloadShow`), status bar (`onStatusBarUrl`), and fullscreen (`onFullscreenChanged`)
