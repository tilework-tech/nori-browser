...
- **Two test suites**: The tests are split into two `test.describe` blocks, each with its own Electron app instance:
  ...
  - `Nori Browser Tabs` -- tab management: tab bar rendering, creating/closing/switching tabs, navigation isolation between tabs, keyboard accelerators (Ctrl+T, Ctrl+W, next-tab), tab reordering via IPC, bridge CLI `list-tabs`, reopen closed tab (Ctrl+Shift+T) with URL and position restoration, middle-click to close tab, duplicate tab, close other tabs, close tabs to the right, tab pinning (pin/unpin rendering, narrow width, ordering invariant, boundary-clamped reordering, close-other/close-right skipping pinned tabs, middle-click and Ctrl+W still closing pinned tabs, duplicate-of-pinned is unpinned, getTabs reporting pinned state), and closing the last tab to close the window
