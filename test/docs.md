...
- **Test suites**: The tests are split into `test.describe` blocks, each with its own Electron app instance and distinct CDP/control port offsets to avoid collisions. Suites cover core functionality, tab management, tab pinning, favicons/loading indicators, omnibox search classification, and Chrome browser features (context menu, DevTools, find-in-page, zoom, downloads, print, fullscreen, status bar, permissions)
...
- Each suite uses its own port offsets (`CDP_PORT + 0`, `+ 10`, `+ 20`, `+ 30`, `+ 40`, `+ 50`) so all suites can run independently without port collisions
